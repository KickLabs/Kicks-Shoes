...
const ShoesPage = () => {
  ...
  const [filters, setFilters] = useState({ productType: 'shoes' }); // Default filter for shoes
  ...
  const fetchProducts = async (newFilters, newPage = 1) => {
    try {
      ...
      // Build query parameters - always include shoes filter
      const params = {
        page: newPage,
        limit: pageSize,
        sortBy,
        order: sortOrder,
        productType: 'shoes', // Force shoes filter
      };
  ...
  const handleFilterChange = newFilters => {
    const updatedFilters = { ...newFilters, productType: 'shoes' };
  ...
  return (
    <div style={{ display: 'flex', padding: 32, gap: 32 }}>
      <FilterSidebar onFiltersChange={handleFilterChange} productType="shoes" />
      ...
      <div style={{ flex: 1 }}>
        ...
        {/* {isVisualSearch && visualSearchResults && (
          <Alert
            type="info"
            showIcon
            message="Kết quả tìm kiếm giày bằng hình ảnh"
            description={
              <div>
                <Text strong>AI đã phân tích hình ảnh giày của bạn:</Text>
        ...
        {loading ? (
          ...
          >
            <Empty description="No shoes found" image={Empty.PRESENTED_IMAGE_SIMPLE} />
          </div>
        ) : (
          <>
            {/* Sort Section */}
