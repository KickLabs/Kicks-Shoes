...
const OtherPage = () => {
  ...
  const [filters, setFilters] = useState({ productType: 'other' }); // Default filter for other products
  ...
  const fetchProducts = async (newFilters, newPage = 1) => {
    try {
      ...
      // Build query parameters - always include other filter
      const params = {
        page: newPage,
        limit: pageSize,
        sortBy,
        order: sortOrder,
        productType: 'other', // Force other filter
      };
  ...
  const handleFilterChange = newFilters => {
    const updatedFilters = { ...newFilters, productType: 'other' };
    ...
    <div style={{ display: 'flex', padding: 32, gap: 32 }}>
      ...
      <div style={{ flex: 1 }}>
        ...
        {loading ? (
          ...
          >
            <Empty description="No other products found" image={Empty.PRESENTED_IMAGE_SIMPLE} />
          </div>
        ) : (
          <>
            {/* Sort Section */}
