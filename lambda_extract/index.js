...
async function processMessageWithBedrock(message) {
  ...
  try {
    ...
    const modelArn = process.env.BEDROCK_MODEL 
      ? `arn:aws:bedrock:${process.env.BEDROCK_REGION}::foundation-model/${process.env.BEDROCK_MODEL}`
      : `arn:aws:bedrock:${process.env.BEDROCK_REGION}::foundation-model/anthropic.claude-3-5-sonnet-20241022-v2:0`;
    ...
    const timestamp = Date.now();
    const errorMessage = {
      pk: `CONV#${message.conversationId || message.pk}`,
      sk: `MSG#${timestamp}`,
      conversationId: message.conversationId || message.pk,
      timestamp: timestamp,
      userId: message.receiver || message.userId,
